...
namespace 
{
	struct transform_component
	{
		...
		transform::init_info to_init_info()
		{
			...
			memcpy(&info.position[0], &position[0], sizeof(f32) * _countof(info.position));
			memcpy(&info.scale[0], &scale[0], sizeof(f32) * _countof(info.scale));
			XMFLOAT3A rot{ &rotation[0] };
			XMVECTOR quat{ XMQuaternionRotationRollPitchYawFromVector(XMLoadFloat3A(&rot)) };//Quaternion_Rotation_Roll_Pitch_Yaw_From_Vector
			XMFLOAT4A rot_quat{};
...
EDITOR_INTERFACE id::id_type CreateGameEntity(game_entity_descriptor* e)
{
	...
	return game_entity::create_game_entity(entity_info).getId();
}
